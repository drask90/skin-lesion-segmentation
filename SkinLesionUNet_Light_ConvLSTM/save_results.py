...
def build_word_doc(fold_results, qualitative_image_path, out_path):
    ...
    doc.add_paragraph(
        "Dataset: ISIC 2016  |  Model: UNet + Bidirectional ConvLSTM Skip Connections (base=16)  |  "
    ...
    # ── Training Setup ─────────────────────────────────────────
    doc.add_heading("1. Architecture & Hyperparameters", level=1)

    doc.add_heading("Model Architecture", level=2)
    doc.add_paragraph(
        "A lightweight U-Net is used where each encoder skip connection "
        "(e1–e4) is processed by a Bidirectional ConvLSTM block before "
        "being concatenated with the decoder path."
    )
    arch_items = [
        "The feature map H-dimension is treated as a sequence of rows.",
        "A forward ConvLSTM cell scans rows 0 → H-1; a backward cell scans H-1 → 0.",
        "Both hidden-state sequences are concatenated (2 × hidden_ch channels) "
        "and projected back to in_ch via 1×1 Conv + BN + ReLU.",
        "A residual connection adds the BiConvLSTM output to the original skip feature map.",
        "Hidden channels per level: e1=8, e2=16, e3=32, e4=64.",
        "Bottleneck: plain DoubleConv (no attention module).",
    ]
    ...
    hyperparams = [
        ("Model",            "UNet + BiConvLSTM skip connections (base=16)"),
        ("Input size",       "256 × 256"),
        ("Optimizer",        "AdamW"),
        ("Learning rate",    "1e-4"),
        ("Epochs",           "200"),
